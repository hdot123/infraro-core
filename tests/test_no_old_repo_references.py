...
def test_no_old_repo_literals():
    ...
    for file_path in tracked_files:
        if file_path and file_path != "tests/test_no_old_repo_references.py":  # Exclude self to avoid false positives
    ...
    assert not found_matches, (
        f"Found references to old repository 'hdot123-org/infra-core' in the codebase:\n" +
        "\n".join(found_matches)
    )
...
def test_no_org_infra_core_references():
    ...
    assert not found_matches, (
        f"Found specific references to old repo 'hdot123-org/infra-core':\n" +
        "\n".join(found_matches)
    )
